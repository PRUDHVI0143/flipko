  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="flex items-center justify-between mb-10">
        ...
        <Link to="/" className="text-slate-500 hover:text-primary-600 font-bold flex items-center gap-2 transition-colors">
      ...
      {items.length === 0 ? (
        <motion.div 
            ...
            className="text-center py-20 bg-white rounded-[40px] shadow-xl shadow-slate-200/50 border border-slate-100"
        >
          <div className="w-24 h-24 bg-slate-50 rounded-3xl flex items-center justify-center mx-auto mb-8">
            <ShoppingBag className="w-12 h-12 text-slate-300" />
          </div>
          <h2 className="text-3xl font-bold text-slate-800 mb-4">Your cart feels lonely</h2>
          ...
          <Link to="/" className="bg-slate-900 text-white px-10 py-4 rounded-2xl font-bold hover:bg-primary-600 transition-all shadow-xl shadow-slate-900/10 hover:-translate-y-1 inline-block">
            Explore Collection
          </Link>
        </motion.div>
      ) : (
        <div className="flex flex-col xl:flex-row gap-12">
            ...
            <AnimatePresence mode="popLayout">
              {items.map((item) => (
                <motion.div 
                    ...
                    exit={{ opacity: 0, x: -20 }}
                    key={item.id} 
                    className="flex flex-col sm:flex-row gap-6 bg-white p-6 rounded-[32px] shadow-sm hover:shadow-xl transition-all duration-500 border border-slate-100 group"
                >
                  {/* Item Image */}
                  <div className="w-full sm:w-40 h-40 bg-slate-50 rounded-2xl flex items-center justify-center p-4 overflow-hidden relative">
                    <div className="absolute inset-0 bg-gradient-to-br from-transparent to-slate-200/20"></div>
                    {item.product.image ? (
                       <img 
                      ...
                      <span className="text-slate-300 font-bold opacity-30 text-xs">NO IMAGE</span>
                    )}
                  </div>
                  ...
                  <div className="flex-1 flex flex-col justify-between py-1">
                    <div>
                      <div className="flex items-start justify-between gap-4 mb-2">
                         <h3 className="font-bold text-xl text-slate-900 leading-tight line-clamp-2">{item.product.name}</h3>
                         <div className="text-2xl font-black text-slate-900">₹{(item.product.price * item.quantity).toFixed(0)}</div>
                      </div>
                      <div className="text-primary-500 font-bold text-xs uppercase tracking-widest mb-4">
                        {item.product.category?.name || 'Standard Edition'}
                      </div>
                      <div className="text-slate-400 text-sm italic">Unit Price: ₹{item.product.price}</div>
                    </div>
                    ...
                    <div className="mt-6 flex items-center justify-between">
                      <div className="flex items-center bg-slate-50 rounded-2xl p-1.5 border border-slate-100">
                        <button 
                            ...
                            className="p-2 hover:bg-white rounded-xl text-slate-600 transition-all active:scale-90"
                        >
                            <Minus className="w-4 h-4" />
                        </button>
                        <span className="px-5 font-black text-slate-900 min-w-[50px] text-center">{item.quantity}</span>
                        <button 
                            onClick={() => handleUpdateQuantity(item.id, item.quantity, 1)}
                            className="p-2 hover:bg-white rounded-xl text-slate-600 transition-all active:scale-90"
                        >
                      ...
                      <button 
                        ...
                        className="text-rose-500 bg-rose-50 p-3 rounded-2xl hover:bg-rose-500 hover:text-white transition-all group-hover:shadow-lg group-hover:shadow-rose-100"
                      >
          ...
          <div className="w-full xl:w-[400px]">
            <motion.div 
                ...
                className="bg-slate-900 text-white p-8 rounded-[40px] shadow-2xl shadow-slate-900/20 sticky top-24"
            >
